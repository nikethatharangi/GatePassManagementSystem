    ...
    public class GatePassListGrAcknPendingModel : PageModel
    {
        ...
        public async Task OnGet()
        {
            try
            {
                ...
                {
                    PersonalGPs = await _db.PersonalGP.Where(gp => gp.UserId == Uid && gp.ASdgm == "A" && gp.ASguard == null && gp.CreateDate.Year == targetLocalTime.Year && gp.CreateDate.Month == targetLocalTime.Month && gp.CreateDate.Date == targetLocalTime.Date).ToListAsync();
                }
                else if(deptId == 10)
                {
                ...
                {
                    PersonalGPs = await _db.PersonalGP.Where(gp => gp.UserId == Uid && gp.AShod == "A" && gp.ASdgm == "A" && gp.ASguard == null && gp.CreateDate.Year == targetLocalTime.Year && gp.CreateDate.Month == targetLocalTime.Month && gp.CreateDate.Date == targetLocalTime.Date).ToListAsync();
                }
            }
            catch (Exception ex)
            {
