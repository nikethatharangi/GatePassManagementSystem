...
{
    public class OtherRsReportModel : PageModel
    {
        ...
        public OtherRsReportModel(ApplicationDbContext db)
        {
            _db = db;
            cm = new Common();
        }
        ...
        public async Task OnGet()
        {
            ...
            PersonalGPs = await _db.PersonalGP.Where(gp => gp.CreateDate.Year == targetLocalTime.Year && gp.CreateDate.Month == targetLocalTime.Month && gp.ChkOther == true).ToListAsync();
            WorkerGPs = await _db.WorkerGP.Where(gp => gp.CreateDate.Year == targetLocalTime.Year && gp.CreateDate.Month == targetLocalTime.Month && gp.ChkOther == true).ToListAsync();
        }
        ...
        public IActionResult OnGetGenerateExcel()
        {
            ...
            PersonalGPs = _db.PersonalGP
                .Where(gp => gp.CreateDate.Year == targetLocalTime.Year && gp.CreateDate.Month == targetLocalTime.Month && gp.ChkOther == true)
                .ToList();
        ...
        public IActionResult GenerateExcel(List<Model.PersonalGP> personalGPs)
        {
            ...
            {
                var worksheet = workbook.Worksheets.Add("OtherReasonReportStaff");
                ...
                worksheet.Cell(1, 7).Value = "Out Time";
                worksheet.Cell(1, 8).Value = "In Time";

                var headerCells = worksheet.Range(1, 1, 1, 8);
                ...
                foreach (var personalGP in personalGPs)
                {
                    ...
                    worksheet.Cell(row, 5).Value = personalGP.Description;
                    worksheet.Cell(row, 6).Value = personalGP.CreateDate;
                    worksheet.Cell(row, 7).Value = personalGP.OutTime.ToString();
                    worksheet.Cell(row, 8).Value = personalGP.InTime.ToString();
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "OtherReasonReportStaff.xlsx");
            }
        ...
        public IActionResult GenerateExcelcsv(List<Model.PersonalGP> personalGPs)
        {
            using (var workbook = new XLWorkbook())
            {
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "OtherReasonReportStaff.csv");
            }
        ...
        public IActionResult GenerateExcelworker(List<WorkerGP> workerGPs)
        {
            using (var workbook = new XLWorkbook())
            {
                var worksheet = workbook.Worksheets.Add("OtherReasonReportWorkers");
                ...
                foreach (var workerGP in workerGPs)
                {
                    ...
                    worksheet.Cell(row, 4).Value = depname;
                    worksheet.Cell(row, 5).Value = workerGP.Reason;
                    worksheet.Cell(row, 6).Value = workerGP.CreateDate;
                    worksheet.Cell(row, 7).Value = workerGP.OutTime.ToString();
                    worksheet.Cell(row, 8).Value = workerGP.InTime.ToString();
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "OtherReasonReportWorkers.xlsx");
            }
        ...
        public IActionResult GenerateExcelcsvworker(List<WorkerGP> workerGPs)
        {
            using (var workbook = new XLWorkbook())
            {
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "OtherReasonReportWorkers.csv");
            }
