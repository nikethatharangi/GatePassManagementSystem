...
{
    public class ViewRecievedRGPModel : PageModel
    {
        public readonly ApplicationDbContext _db;
        private readonly INotyfService _notify;
        public readonly Common cm;

        public ViewRecievedRGPModel(ApplicationDbContext db, INotyfService notyf)
        {
            _db = db;
            cm = new Common();
            _notify = notyf;
        ...
        public async Task OnGetAsync()
        {
            try
            {
                ...
                ReturnableGPs = await _db.ReturnableGP.Where(gp => gp.ToDept == deptId && gp.CreateDate.Year == targetLocalTime.Year && gp.CreateDate.Month == targetLocalTime.AddMonths(3).Month && (gp.Satisfied == true || gp.Satisfy == true)).ToListAsync();

            }
            catch (Exception ex)
            {
                cm.Logwrite("Error in ViewRecievedRGPModel OnGet method :" + ex.Message);
            }
        }
        ...
        public IActionResult OnPostHODSatis(int id, string chkNotSatis, string txtHODRemarks, string chkSatis)
        {
            try
            {
                ...
                if (updatedpgp != null)
                {
                    ...
                    return RedirectToPage("ViewRecievedRGP");
                }
                else
                {
                    cm.Logwrite("Error in OnPostApproveChange method in ViewRecievedRGPModel: if()");
                }
            }
            catch (Exception ex)
            {
                cm.Logwrite("Error in OnPostApproveChange method in ViewRecievedRGPModel: " + ex.Message);
            }
